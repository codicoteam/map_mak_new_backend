...
const BulkBuyingService = {
  ...
  async getAllBulkBuying(page = 1, size = 10) {
    ...
    const bulkBuyings = await BulkBuying.find().skip(skip).limit(size);
  ...
  async searchCategory(categoryName, page = 1, size = 10) {
    const skip = (page - 1) * size;
    const bulkBuyings = await BulkBuying.find({ "bulkBuyingCategories.name": categoryName }).skip(skip).limit(size);
  ...
  async searchType(type, page = 1, size = 10) {
    const skip = (page - 1) * size;
    const bulkBuyings = await BulkBuying.find({ "bulkBuyingAttributes.type": type }).skip(skip).limit(size);
